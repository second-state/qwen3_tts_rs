...
impl CodePredictor {
    ...
    pub fn load(
        ...
        println!("  Loaded {} code_predictor embeddings", code_embeddings.len());
        ...
        let norm_weight = weights
            ...
            .ok_or_else(|| {
                Qwen3TTSError::ModelLoad("Missing code_predictor norm.weight".into())
            })?
            .to_device(device)
            .to_kind(Kind::Float);
    ...
    pub fn generate_codes(
        ...
        let mut sequence = Tensor::cat(&[main_hidden.shallow_clone(), code_0_embedding.shallow_clone()], 1);
        ...
        for step in 0..self.lm_heads.len() {
            ...
            let upper =
                Tensor::ones([seq_len, seq_len], (Kind::Bool, self.device)).triu(1);
...
impl TalkerModel {
    ...
    pub fn load(
        ...
        let code_predictor =
            CodePredictor::load(weights, &config.code_predictor_config, device)?;
    ...
    pub fn build_input_embeddings(
        ...
        let text_with_eos = Tensor::cat(
            &[text_content_embed, tts_eos_embed.shallow_clone()],
            1,
        ); // [1, N+1, 1024]

        // Codec side: codec_pad repeated (N+1) times
        let codec_pad_embed = self.embed_codec(&[codec_pad_id]); // [1, 1, 1024]
        let codec_pad_repeated = codec_pad_embed
            .expand([1, (num_text_tokens + 1) as i64, self.hidden_size], false);
        ...
        let input_embeddings = Tensor::cat(
            &[role_embed, phase2, phase3, phase4],
            1,
        );
    ...
    pub fn generate_codes(
        ...
        for step in 0..max_codes {
            ...
            let normed_hidden =
                self.forward_embeds(&full_sequence, Some(&causal_mask));
            ...
            let main_hidden = normed_hidden
                .select(1, seq_len - 1)
                .unsqueeze(1); // [1, 1, hidden_size]
...
impl TTSInference {
    ...
    pub fn generate_with_params(
        ...
        println!(
            ...
            codec_eos_id, codec_think_id, codec_think_bos_id, codec_think_eos_id,
        ...
        println!("Generating audio codes (temp={}, top_k={}, max={})...", temperature, top_k, max_codes);
        ...
        if codes.is_empty() {
            ...
            waveform = Vec::<f32>::try_from(
                audio_tensor.view([audio_len as i64]).to_kind(Kind::Float),
            )
            ...
            let codebook_size =
                self.config.talker_config.code_predictor_config.vocab_size as f32;
